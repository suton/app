...
//            array('gallery_id, rank', 'numerical', 'integerOnly' => true),
    ...
    {
        return Yii::app()->request->baseUrl . '/' . $this->galleryDir . '/' . $this->gallery_id . '/_' . $this->getFileName('') . '.' . $this->galleryExt;
    }
    ...
    {
        return Yii::app()->request->baseUrl . '/' . $this->galleryDir . '/' . $this->gallery_id . '/' . $this->getFileName($version) . '.' . $this->galleryExt;
    }

    public function setImage($path)
    {
        //save image in original size
        Yii::app()->image->load($path)->save(Yii::getPathOfAlias('webroot') . '/' . $this->galleryDir . '/' . $this->gallery_id . '/' . $this->getFileName('') . '.' . $this->galleryExt);
        //create image preview for gallery manager
        Yii::app()->image->load($path)->resize(300, null)->save(Yii::getPathOfAlias('webroot') . '/' . $this->galleryDir. '/' . $this->gallery_id  . '/_' . $this->getFileName('') . '.' . $this->galleryExt);
    ...
    {
        $this->removeFile(Yii::getPathOfAlias('webroot') . '/' . $this->galleryDir . '/' . $this->gallery_id . '/' . $this->getFileName('') . '.' . $this->galleryExt);
    ...
    public function removeImages()
    {
        foreach ($this->gallery->versions as $version => $actions) {
            $this->removeFile(Yii::getPathOfAlias('webroot') . '/' . $this->galleryDir . '/' . $this->gallery_id . '/' . $this->getFileName($version) . '.' . $this->galleryExt);
        }
    }
    ...
    public function updateImages()
    {
        foreach ($this->gallery->versions as $version => $actions) {
            ...
            $image = Yii::app()->image->load(Yii::getPathOfAlias('webroot') . '/' . $this->galleryDir . '/' . $this->gallery_id . '/' . $this->getFileName('') . '.' . $this->galleryExt);
            foreach ($actions as $method => $args) {
                call_user_func_array(array($image, $method), is_array($args) ? $args : array($args));
            }
            $image->save(Yii::getPathOfAlias('webroot') . '/' . $this->galleryDir . '/' . $this->gallery_id . '/' . $this->getFileName($version) . '.' . $this->galleryExt);
        }
    }
    ...
    private function getSize($version = '')
    {
        if (!isset($this->_sizes[$version])) {
            $path = Yii::getPathOfAlias('webroot') . '/' . $this->galleryDir . '/' . $this->gallery_id . '/' . $this->getFileName($version) . '.' . $this->galleryExt;
