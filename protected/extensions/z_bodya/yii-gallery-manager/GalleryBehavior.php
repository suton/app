...
class GalleryBehavior extends CActiveRecordBehavior
{
    ...
    public function beforeSave($event)
    {
        ...
        if ($event->isValid) {
            if (empty($this->getOwner()->{$this->idAttribute})) {
                ...
                //@prasuton create sub folder in gallery/sub
                $this->createSubFolder($gallery->id);
            }
        }
    ...
    public function createSubFolder($subFolderId){
        $galleryPhoto=new GalleryPhoto();
        $galleryDir=$galleryPhoto->galleryDir;
        $path=$galleryDir.'/'.$subFolderId;
